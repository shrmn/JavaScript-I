...
let lastCar = 0;
findLastCar = function(){
  let result = [];
  carMake = inventory[inventory.length - 1]["car_make"];
  result.push(carMake);
  carModel = inventory[inventory.length - 1]["car_model"];
  result.push(carModel);
  let lastCar = result.join(' ');  
  return lastCar;
};
...
let carModels = [];
listCarModels = function() {
  for (var i = 0; i < inventory.length; i++) {
    carModels.push(inventory[i]["car_model"]);
  }
  return carModels.sort();
}
console.log(listCarModels());

// ==== Challenge 4 ====
// The accounting team needs all the years from every car on the lot. Create a new array from the dealer data containing only the car years and log the result in the console.
let carYears = [];
listCarYears = function() {
  for (var i = 0; i < inventory.length; i++) {
    carYears.push(inventory[i]["car_year"]);
  }
  return carYears;
}
console.log(listCarYears());

// ==== Challenge 5 ====
// The car lot manager needs to find out how many cars are older than the year 2000. Using the carYears array you just created, find out how many cars were made before the year 2000 by populating the array oldCars and logging it's length.
let oldCars =[];
oldEnough = function(year) {
  return year < 2001;
}
findOldCars = function() {
  return carYears.filter(oldEnough);  
}
console.log(findOldCars()); 

// ==== Challenge 6 ====
// A buyer is interested in seeing only BMW and Audi cars within the inventory.  Return an array that only contains BMW and Audi cars.  Once you have populated the BMWAndAudi array, use JSON.stringify() to show the results of the array in the console.
let BMWAndAudi =[];
function listFancyCars() {
